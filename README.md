...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/bluesky_get_profile \
  -H 'Content-Type: application/json' \
  -d '{"handle":"alice.bsky.social"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/bluesky_get_profile`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
